...
fn test_output_is_valid_json() {
    ...
    // Use a temp dir provided by the OS rather than hardcoding /tmp
    let out_dir = std::env::temp_dir();
    let out_base = out_dir.join("test_map_rust_0");
    let out_json = out_dir.join("test_map_rust_0.json");

    let output = Command::new(binary)
        .args(["--seed", "0", "--output", out_base.to_str().unwrap()])
        .output()

...
fn build_render_packet(draw_data: &MapDrawData) -> Result<WasmRenderPacket, JsValue> {
    ...
    let heightmap = draw_data
        .heightmap
        .as_ref()
        .ok_or_else(|| JsValue::from_str("render packet requires heightmap data"))?;
    let land_mask = draw_data
        .land_mask
        .as_ref()
        .ok_or_else(|| JsValue::from_str("render packet requires land_mask data"))?;

    if heightmap.width != land_mask.width || heightmap.height != land_mask.height {
        return Err(JsValue::from_str(
            "heightmap and land_mask dimensions must match",
        ));
    }

    let terrain_width = heightmap.width;
    let terrain_height = heightmap.height;
    let flux_map = draw_data.flux_map.as_ref();
    ...
    let metadata_json = serde_json::to_string(&RenderPacketMetadata {
        ...
        city_count: (draw_data.city.len() / 2) as u32,
        town_count: (draw_data.town.len() / 2) as u32,
        river_count: draw_data.river.len() as u32,
        territory_count: draw_data.territory.len() as u32,
        label_count: draw_data.label.len() as u32,
    })
    ...
    let (label_bytes, label_offsets, label_anchors, label_sizes) =
        encode_labels(draw_data, terrain_width, terrain_height, &elevations);
    let (land_polygon_positions, land_polygon_offsets) = encode_land_polygons(draw_data);
    ...
    Ok(WasmRenderPacket {
        ...
        flux_texture,
        albedo_texture,
        terrain_albedo_texture: surface_textures.terrain_albedo,
        roughness_texture: surface_textures.roughness,
        ao_texture: surface_textures.ao,
        water_color_texture: surface_textures.water_color,
        water_alpha_texture: surface_textures.water_alpha,
        coast_glow_texture: surface_textures.coast_glow,
        slope_segments,
        river_positions,
        river_offsets,
        contour_positions,
